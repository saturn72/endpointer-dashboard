...
function getDefaultValues(datasource: Datasource): Values {
  return {
    name: datasource.name ?? '',
    handle: datasource.handle ?? '',
    category: datasource.category ?? '',
    type: datasource.type ?? 'physical',
    description: datasource.description ?? '',
    tags: datasource.tags ?? '',
    currency: datasource.currency ?? 'USD',
    price: datasource.price ?? 0,
    images: datasource.images ?? [],
    sku: datasource.sku ?? '',
    barcode: datasource.barcode ?? '',
    quantity: datasource.quantity ?? 0,
    backorder: datasource.backorder ?? false,
    height: datasource.height ?? 0,
    width: datasource.width ?? 0,
    length: datasource.length ?? 0,
    weight: datasource.weight ?? 0,
  };
}

export interface DatasourceEditFormProps {
  datasource: Datasource;
}

export function DatasourceEditForm({ datasource }: DatasourceEditFormProps): React.JSX.Element {
  ...
  const {
    ...
    setValue,
    watch,
  } = useForm<Values>({ defaultValues: getDefaultValues(datasource), resolver: zodResolver(schema) });
            ...
            <Stack spacing={2}>
              {handle ? (
                <Typography color="primary.main" variant="subtitle2">
                  https://domain.com/datasources/{handle}
                </Typography>
              ) : (
